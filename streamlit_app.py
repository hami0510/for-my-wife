...
# 2. CSS: UI 최적화 (성경 구절 줄바꿈 및 사이드바 정렬)
st.markdown("""
    <style>
...
bible_list = [
    ...
    ("하나님이 우리에게 주신 것은 두려워하는 마음이 아니요 오직 능력과 사랑과...", "디모데후서 1:7"),
...
with st.sidebar:
    ...
    with st.expander("💌 태교 편지함"):
        letter = st.text_area("이레에게...", key="la", placeholder="오늘의 기록을 남겨주세요")
        if st.button("편지 저장"):
...
if "messages" not in st.session_state:
    st.session_state.messages = [{"role": "assistant", "content": f"안녕 이레 엄마! 무엇이든 물어봐요. 🥰"}]
...
if prompt := st.chat_input("증상, 음식, 약물 등 무엇이든 물어보세요..."):
    ...
    with st.chat_message("assistant"):
        sys_msg = {
            ...
            1. 음식/음료/약물 검수: 절대 금지는 [❌ 절대 금지] 강력 경고 및 대체품 추천. 주의는 [⚠️ 주의] 안내.
            2. 마더세이프(MotherSafe) 가이드 기반.
            3. 마지막은 항상 "사랑해"."""
        }

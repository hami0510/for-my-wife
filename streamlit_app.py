...
st.markdown("""
    ...
    .bible-box { 
        ...
        color: #555555 !important; line-height: 1.6;
    }
...
with st.sidebar:
    ...
    st.markdown(f"""
        <div class="sb-box">
            ...
            <b style="color:#ff6b6b; font-size:1.2rem;">D-{d_day if d_day > 0 else 'Day!'}</b>
        </div>
    """, unsafe_allow_html=True)
...
if "messages" not in st.session_state:
    st.session_state.messages = [{"role": "assistant", "content": f"안녕 이레 엄마! 현재 {current_weeks}주차네. 오늘 컨디션은 어때? 궁금한 건 무엇이든 물어봐. 🥰"}]
...
if prompt := st.chat_input("증상을 물어보거나 대화를 나눠보세요..."):
    ...
    with st.chat_message("assistant"):
        sys_msg = {"role": "system", "content": f"산부인과 전문의 이레 아빠야. 현재 {current_weeks}주차인 아내에게 다정하게 답해줘. 사랑한다고 말해줘."}
